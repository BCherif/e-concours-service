...
public class CandidacyService extends ECDefaultBaseService<Candidacy, CandidacyRepository> {
    ...
    public CandidacyService(CandidacyRepository repository, ECEntityManager manager, ECLogger logger, CompetitionRepository competitionRepository, CandidateService candidateService, AttachmentRepository attachmentRepository, UploadFileService uploadFileService, NotificationService notificationService, CandidateRepository candidateRepository) {
    ...
    public ECResponse<?> create(CandidacySaveEntity candidacySaveEntity, MultipartFile[] files) {
        try {
            ...
            if (optionalCompetition.isPresent()) {
                Optional<Candidacy> candidacyOptional = repository.findByCandidateUidAndCompetitionUid(candidacySaveEntity.getCandidateId(), candidacySaveEntity.getCompetitionId());
...
//                    String[] tab = Objects.requireNonNull(file.getOriginalFilename()).split("\\.");
    ...
    public ECResponse<?> getCandidacyByCandidate(String candidateUid, Pageable pageable) {
        try {
            PageData<Candidacy> pageData = PageData.fromPage(repository.findAllByCandidateUid(candidateUid, pageable));
